...
pattern = r"\[(.*?), (.*?), (.*?), (.*?), (.*?), (.*?)\]"  
path_snapping_list = {}
def path_snapping(CoordOperator):
    """ Snaps the operator's path to the ideal path, and returns the new path. Incert [[x[j]],[y[j]],[z[j]]] coordinates of the operator's path."""
    ...
    for i in range(len(ScriptIdeal)):
        if "movel" not in ScriptIdeal[i] and "movej" not in ScriptIdeal[i]:
            ...
            path_snapping_list[i] = ScriptIdeal[i]
